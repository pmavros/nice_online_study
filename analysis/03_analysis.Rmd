...
t1 %>% 
  ...
  footnote(number  = "Bayesian t-tests; BF = bayes factor.") %>% 
...
```{r eval=TRUE, fig.height=fig.width*1.5, message=FALSE, warning=FALSE, cache=FALSE, include=FALSE}
...
p <- (plot_distributions(df, "Crowdedness") |
  ...
  (plot_distributions(df, "WillingnessToWalk") |
    plot_distributions(df, "Structure")) /
  (plot_distributions(df, "Scenic") |
    plot_distributions(df, "Interest")) /
  (plot_distributions(df, "Width") |
    plot_distributions(df, "Familiarity")) /
  (plot_distributions(df, "Valence") |
    plot_distributions(df, "Arousal"))
p
