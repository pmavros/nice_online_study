...
dat$Outcome <- recode(dat$Outcome, "Attractiveness"="Willingness-to-walk")

p_psych <- preds |>
  dplyr::filter(Predicted <= 7) |>
  mutate(Beauty = fct_rev(as.factor(Beauty)),
         Outcome = recode_factor(Outcome, "Attractiveness"="Willingness-to-walk"),
         Outcome = fct_relevel(Outcome, c("Willingness-to-walk", "Positivity", "Excitement", "Restoration", "Interest", "Presence"))) |>
  ...
  ggside::theme_ggside_void() +
  facet_wrap(~Outcome, strip.position = "left")
