...
plot_distributions <- function(df, col) {
  ...
  p2 <- df[c("Video", col)] |> 
    ...
    theme(axis.text.y = element_blank(), axis.title.x = element_blank()) +
    labs(y = "Videos") +
    ggside::geom_xsidedensity(aes(y=stat(density)), fill = "#4CAF50", color=NA) +
    ggside::geom_xsidedensity(data=df, aes_string(x=col, y="stat(density)"), fill = "black", color=NA, alpha=0.2) +
    ggside::theme_ggside_void()
  
  wrap_elements(
    p1 + p2 +
    plot_annotation(title = col,
...
```{r fig.height=fig.width*1.5, message=FALSE, warning=FALSE, cache=FALSE}
...
for(i in 1:length(vars)) { 
  ...
  f <- paste0(outcome_var," ~ log1p(mean_pedcounts)* ", var, " + (", var, "|Participant)")
  ...
  dfsub <- modelbased::estimate_grouplevel(model) |> 
    ...
    group_by(Participant) |> 
    slice(1) |> 
    data_rename(pattern = paste0("Participant_Coefficient_", var), paste0(outcome_var, "_", var), regex=TRUE) |> 
  ...
  # Predicted
  pred <- estimate_relation(model, at=c("mean_pedcounts", var), length=c(30, 3))
...
p_crowd <- preds |> 
  ...
  geom_line(aes(color = Value, group=Value)) +
  geom_text(data=slice(group_by(preds, Variable), 1), 
            aes(x = 0, y = 7, label = Label), hjust=0, vjust=1) +
  scale_fill_gradientn(colours=c("white", "#4CAF50"), guide="none") +
  # scale_color_gradientn(colours=c("#2196F3", "#9C27B0", "red")) +
  scale_colour_viridis_b(option = "A") +
  scale_x_continuous(expand = c(0, 0)) +
  scale_y_continuous(expand = c(0, 0), breaks = c(0, 1,3,5,7)) +
  labs(x  = "Pedestrian counts (log)", y = "Perceived Crowdedness (1-7)") +
  theme_modern() +
  theme(strip.placement = "outside", strip.text.x = element_text(hjust = 0)) +
  ggside::geom_xsidedensity(data=df, aes(y=stat(density)), fill = "grey") +
  ggside::theme_ggside_void() +
  facet_wrap(~Variable, strip.position = "top", ncol=2) 

p_crowd

ggsave(p_crowd, filename = "output/plots_perc_crowd.jpg", scale = 1.5, width = 12, height = 12, units = "cm", dpi = 300)
...
names(mods) <- vars
modelsummary(mods, stars = TRUE,  statistic = "[{conf.low} - {conf.high}]",
            coef_omit = "SD|Cor", 
...
dat <- data.frame()
mods <- list()
vars <-  c( "Structure" , "Scenery", "Familiarity","Width")

for(i in 1:length(vars)) {
  var = vars[i]
  print(var)
  
  f <- paste0("Beauty ~ Typology + poly(mean_pedcounts, 2) * ", var, " + (", var, "|Participant)")
  ...
  # Predicted
  pred <- estimate_relation(model, at=c("mean_pedcounts", var), length=c(3, 30))
  names(pred)[names(pred) == var] <- "Value"
  pred$Variable <- var
  pred$Label <- paste0("Objective Crowding: ", format_p(param$p[2]), 
...
p_beauty <- preds |> 
  ...
  geom_line(aes(color = mean_pedcounts, group=mean_pedcounts)) +
  geom_text(data=slice(group_by(preds, Variable), 1), 
            aes(x = 0, y = 7, label = Label), hjust=0, vjust=1) +
  scale_fill_gradientn(colours=c("white", "#2196F3"), guide="none") +
  scale_color_gradientn(colours=c("orange", "red", "purple")) +
  scale_x_continuous(expand = c(0, 0)) +
  scale_y_continuous(expand = c(0, 0)) +
  labs(colour  = "Average \npedestrian \ncounts", y = "Beauty") +
  theme_modern() +
  ...
  # ggside::geom_xsidedensity(data=df, aes(y=stat(density)), fill = "grey") +
  # ggside::theme_ggside_void() +
  facet_wrap(~Variable, strip.position = "bottom", ncol=2, scales="free_x") 

p_beauty

ggsave(p_beauty, filename = "output/plots_perc_beauty.jpg", scale = 1.5, width = 12, height = 12, units = "cm", dpi = 300)
```
...
df <- df %>% filter(!is.na(Crowdedness))
...
for(i in 1:length(vars)) {
  ...
  f <- paste(resp, " ~ Typology / (poly(mean_pedcounts, 2) * Beauty) + (1|Participant) + (1|Video)")
  ...
  # Predictions
  preds <- estimate_relation(model, length=c(20, 3)) |> 
    mutate(Outcome = resp,
           Label_urban = paste0("Urban",
                                "\nCrowdedness: ", format_p(param$p[3]),
                                "\nBeauty: ", format_p(param$p[5]),
                                "\nInteraction: ", format_p(param$p[7])),
           Label_green = paste0("Green: ", format_p(param$p[2]),
                                "\nCrowdedness: ", format_p(param$p[4]),
                                "\nBeauty: ", format_p(param$p[6]),
                                "\nInteraction: ", format_p(param$p[8]))) |> 
    rbind(preds) 
  
  # Data for density
  dat <- rbind(dat, data.frame(mean_pedcounts = df$mean_pedcounts, Predicted = df[[resp]], Outcome= resp))
...
# re-order results for better ploting
preds <- preds %>% mutate(Outcome = factor(Outcome, levels = c("Attractiveness", "Positivity", "Excitement", "Restoration", "Interest", "Presence" ) ) )
...
p_psych <- ggplot(preds, aes(x = mean_pedcounts, y = Predicted)) +
  stat_density_2d(data=dat, aes(fill = ..density..), geom = "raster", contour = FALSE) +
  geom_line(aes(color = Beauty, linetype = Typology, group=interaction(Beauty, Typology))) +
  geom_text(data=slice(group_by(preds, Outcome), 1), 
            aes(x = 0, y = 8.8, label = Label_urban), hjust=0, vjust=1, size=3) + 
  geom_text(data=slice(group_by(preds, Outcome), 1), 
            aes(x = 4, y = 8.8, label = Label_green), hjust=0, vjust=1, size=3) + 
  scale_fill_gradientn(colours=c("white", "#FFC107"), guide="none") +
  # scale_color_gradientn(colours=c("#795548", "#9C27B0")) +
  scale_colour_viridis_b() +
  scale_x_continuous(expand = c(0, 0)) +
  scale_y_continuous(expand = c(0, 0),  breaks = c(1,3,5,7)) +
  theme_modern() +
  theme(strip.placement = "outside", strip.text.x = element_text(hjust = 0)) +
  labs(x = "Average pedestrian count", y = "Score") +
  ggside::geom_xsidedensity(data=dat, aes(y=stat(density)), fill = "grey") +
  # ggside::geom_ysidedensity(data=dat, aes(x=stat(density)), fill = "grey") +
  # ggside::theme_ggside_void() +
  facet_wrap(~ Outcome, strip.position = "top") 

p_psych

ggsave(p_psych, filename = "output/plots_psych_reaction.jpg", scale = 1.5, width = 12, height = 12, units = "cm", dpi = 300)
