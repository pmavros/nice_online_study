...
data_green <- df_stim %>% 
  ...
  filter(Video_duration >= 30) %>% 
  filter(str_detect(tolower(name_df), "vancouver|green corridor", negate = TRUE)) #%>% 
  select(rowid, Perceived_Crowdedness, Beauty) 
...
final_set_urban <- find_most_dissimilar(data_urban, 20)
final_set_green<- find_most_dissimilar(data_green, 20)
...
## inspect final set
```{r}
final_set %>% filter(!is.na(cluster), Typology == "green") %>% select(name_df_keep) %>% print(n = 100)
```
...
final_set %>% 
  # filter(Typology != "green") %>% 
  arrange(Beauty) %>% 
  ggplot(aes(Beauty, Perceived_Crowdedness, linetype = Typology, colour = !is.na(cluster))) +
  geom_point() +
  theme_blackboard() +
  ggdist::geom_slab() +
