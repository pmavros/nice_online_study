...
# 
# plot(parameters::n_factors(data, cor=as.matrix(r)))
# 
# fa <- parameters::factor_analysis(data, n=2, rotation = "oblimin", sort=TRUE)
# 
# fa
# 
# df <- df |>
#   cbind(
#     predict(fa,
#       newdata = select(df, Crowdedness, Beauty, Restoration, Presence,
#                        Attractiveness, Structure, Scenery, Interest, Width,
#                        Familiarity, Positivity, Excitement),
#       names = c("Appeal", "Liveliness", "Familiarity")
#     )
#   )

```

### Clustering and Distance {.tabset}
...
data <- df_stim %>% 
  ...
  # filter(str_detect(tolower(name_df), "vancouver"))
...
dissimilarity_clusters <- dissimilarity_clusters[order(dissimilarity_clusters$dissimilarity), ]

# split object
reslist <- split(dissimilarity_clusters, f=dissimilarity_clusters$cluster)
reslist
## takes first three items with highest overall dissim.
# lapply(reslist, head, n=1)

## returns id´s with highest overall dissimilarity
...
# final_set <- final_set %>% 
#   select(videoname, Typology, Perceived_Crowdedness, Pedestrian_flow, Beauty, Structure, Interest)
...
final_set %>% 
  ...
  theme_blackboard() +
  ggdist::geom_slab() +
  ggside::geom_xsidehistogram(binwidth = 1) +
  ggside::geom_ysidehistogram(binwidth = 1)
